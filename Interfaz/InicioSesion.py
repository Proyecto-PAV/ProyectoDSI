...
class Ui_InicioSesion(object):
    def menu(self, event):
        ...
        contra = self.LE_contra.text()
        cnxn = conexion()
        cursor = cnxn.cursor()
        cursor.execute('SELECT nombre_usuario, contraseña FROM usuarios WHERE nombre_usuario = ? and contraseña = ?', user, contra)
        row = cursor.fetchone()
        if row == None:
            self.lbl_error.setStyleSheet("background-color: red")
            self.lbl_error.setText("Los datos ingresados son correctos")
        else:
            self.menu = QDialog()
            self.ui = Ui_MenuRV()
